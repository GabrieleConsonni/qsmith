...
- `operations_api` manages the operation catalog used by test suites and mock servers.
...
## Main Data Flow
1. UI editors load operation catalog plus datasource/broker context from `app/ui/elaborations_shared`.
...
## Key Modules
- `app/elaborations/api`: supported FastAPI routes for operations, test suites, suite executions, SSE.
...
- `app/ui/elaborations_shared`: shared Streamlit catalog loaders, state keys, operation editor reused by suite/mock-server UIs.
